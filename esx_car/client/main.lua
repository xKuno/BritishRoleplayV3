...
function OpencarShop(shop)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_shop_confirm', {
			title    = _U('car_shop_confirm', data.current.name, ESX.Math.GroupDigits(data.current.price)),
			align    = 'top-right',
			elements = {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_car:buycar', function(bought)
					if bought then
						ESX.ShowNotification(_U('car_shop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)))
						...
						ESX.ShowNotification(_U('car_shop_nomoney'))
						menu2.close()
					end
				end, props)
			else
...
function OpencarGarage(garage)
	ESX.TriggerServerCallback('esx_car:getGarage', function (ownedcars)
		if #ownedcars == 0 then
			ESX.ShowNotification(_U('garage_nocars'))
		else
			-- get all available cars
			local elements = {}
			for i=1, #ownedcars, 1 do
				ownedcars[i] = json.decode(ownedcars[i])

				table.insert(elements, {
					label = getVehicleLabelFromHash(ownedcars[i].model),
					vehicleProps = ownedcars[i]
				})
			end

			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_garage', {
				title    = _U('garage'),
				align    = 'top-right',
				elements = elements
			}, function (data, menu)
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
					TriggerServerEvent('esx_car:takeOutVehicle', vehicleProps.plate)
...
function OpenLicenceMenu(shop)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'car_license', {
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_car:buycarLicense', function (boughtLicense)
				if boughtLicense then
					ESX.ShowNotification(_U('license_bought', ESX.Math.GroupDigits(Config.LicensePrice)))
					menu.close()

					OpencarShop(shop) -- parse current shop
				else
...
function StorecarInGarage(vehicle, teleportCoords)
	local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)

	ESX.TriggerServerCallback('esx_car:storeVehicle', function (rowsChanged)
