...
Citizen.CreateThread(function()
 	while true do
		...
		if IsControlPressed(0, Keys["SHIFT"]) then
			...
			if IsControlJustReleased(0, Keys["4"]) then -- Left Nac Nac! | numpad 6
				repeat
				fnTable[1]();
				Utils.Sleep(fnTable[7]);
				ESX.ShowNotification("Stunt Ready: Left Nac Nac!")
				until not IsControlJustReleased(0, Keys["4"])




			elseif IsControlJustReleased(0, Keys["6"]) then -- Right Nac Nac | numpad 4
				repeat
				fnTable[2]();
				Utils.Sleep(fnTable[7]);
				ESX.ShowNotification("Stunt Ready: Right Nac Nac!")
				until not IsControlJustReleased(0, Keys["6"])



			elseif IsControlJustReleased(0, Keys["7"]) then -- No Hand Zone | numpad 5
				repeat
				fnTable[3]();
				Utils.Sleep(fnTable[7]);
				ESX.ShowNotification("Stunt Ready: No Hand Zone!")
				until not IsControlJustReleased(0, Keys["7"])





			elseif IsControlJustReleased(0, Keys["8"]) then -- numpad 8
				repeat
				fnTable[4]();
				Utils.Sleep(fnTable[7]);
				ESX.ShowNotification("Stunt Ready: Hop Scotch!")
				until not IsControlJustReleased(0, Keys["8"])




				
			elseif IsControlJustReleased(0, Keys["9"]) then -- numpad 7
				repeat
				fnTable[5]();
				Utils.Sleep(fnTable[7]);
				ESX.ShowNotification("Stunt Ready: Egyptian!")
				until not IsControlJustReleased(0, Keys["9"])
			end
		end
		end
		end
