            ...
            if craftingQueue[1] ~= nil then
                if not Config.CraftingStopWithDistance or (Config.CraftingStopWithDistance and isNearWorkbench()) then
                    ...
                    if craftingQueue[1].time == 0 then
                        TriggerServerEvent("brp_play2:itemCrafted", craftingQueue[1].item, craftingQueue[1].count)
...
RegisterNetEvent("brp_play2:craftStart")
AddEventHandler(
    "brp_play2:craftStart",
    function(item, count)
...
RegisterNetEvent("brp_play2:sendMessage")
AddEventHandler(
    "brp_play2:sendMessage",
    function(msg)
        SendTextMessage(msg)
    end
)
...
RegisterNUICallback(
    ...
    function(data)
        ...
        TriggerServerEvent("brp_play2:craft", item, false)
    end
)
