		...
		'<div id="menu_{{_namespace}}_{{_name}}" class="menu{{#align}} align-{{align}}{{/align}}">' +
			'<div class="head"><span>{{{title}}}</span></div>' +
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}} {{#last}}last-elem{{/last}}">' +
	...
	ESX_MENU.render = function() {
		...
		for (let namespace in ESX_MENU.opened) {
			for (let name in ESX_MENU.opened[namespace]) {
				...
				//view.elements[view.elements.length-1].last = 'last-elem';
				for(var counter = 0; counter < view.elements.length; counter++)
				{
					view.elements[counter].last = false;
					if(counter+1 == view.elements.length)
					{
						view.elements[counter].last = true;
						console.log(JSON.stringify(view.elements[counter]));
					}
				}
				console.log(JSON.stringify(view));
				console.log("TAMANHO: " + view.elements.length);
