...
RegisterNetEvent('brp_tv:update')
AddEventHandler('brp_tv:update', function(players)
...
CreateThread(function()
    ...
    TriggerServerEvent('brp_tv:fetch')
...
RegisterCommand('tv', function(source, args)
    if args[1] then
        for k, v in pairs(Config['Objects']) do
            ...
            if DoesEntityExist(obj) then
                TriggerServerEvent('brp_tv:add', args[1], v['Object'], GetEntityCoords(obj), v['Scale'], v['Offset'])
                break
            end
        end
    end
            ...
            if volume >= 0 then
                ...
                for k, v in pairs(data) do
                    if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
                        ...
                        TriggerServerEvent('brp_tv:setvolume', k, volume)
                        break
                    end
                end
...
RegisterCommand('destroy', function(src, args)
    for k, v in pairs(data) do
        if #(GetEntityCoords(PlayerPedId()) - v['Coords']) <= 5.0 then
            TriggerServerEvent('brp_tv:destroy', k)
            break
        end
    end
end)

RegisterNetEvent('brp_tv:delete')
AddEventHandler('brp_tv:delete', function(id)
    if data[id] then
        if data[id]['DUI'] then
            DestroyDui(data[id]['DUI']['Long'])
        end
...
RegisterNetEvent('brp_tv:updatevolume')
AddEventHandler('brp_tv:updatevolume', function(id, volume)
    if data[id] then
