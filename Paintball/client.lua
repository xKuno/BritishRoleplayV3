...
RegisterNetEvent('brp_paintball:queueInfo')
AddEventHandler('brp_paintball:queueInfo', function(text, other)
    queueText = text
    matchends = other
end)

RegisterNetEvent('brp_paintball:hudNotify')
AddEventHandler('brp_paintball:hudNotify', function(text)
    SetNotificationTextEntry('STRING')
    AddTextComponentSubstringPlayerName(text)
    DrawNotification(false, true)
end)

RegisterNetEvent('brp_paintball:start')
AddEventHandler('brp_paintball:start', function()
    queuing = false 
...
RegisterNetEvent('brp_paintball:stop')
AddEventHandler('brp_paintball:stop', function()
...
RegisterNetEvent('brp_paintball:matchOver')
AddEventHandler('brp_paintball:matchOver', function(winner, me)
...
RegisterNetEvent('brp_paintball:died')
AddEventHandler('brp_paintball:died', function(killedBy)
    deaths = deaths + 1
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.JoinCircle, true) <= 7.5 then
                if queueText ~= Config.Translations['match_progress'] then
                    ...
                    if IsControlJustReleased(0, 38) then
                        queuing = not queuing
                        TriggerServerEvent('brp_paintball:join')
                    end
                else
                    ...
                    if GetEntityType(ped) == 1 then
                        for k, v in pairs(GetActivePlayers()) do
                            if GetPlayerPed(v) == ped then
                                TriggerServerEvent('brp_paintball:kill', GetPlayerServerId(v))
                                kills = kills + 1
                                Wait(500)
                                break
                            end
