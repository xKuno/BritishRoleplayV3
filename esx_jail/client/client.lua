...
AddEventHandler("esx:playerLoaded", function(newData)
	...
	ESX.TriggerServerCallback("eswerwex_jwerwail:retrievwereJwerailTiewrweme", function(inJail, newJailTime)
		if inJail then
...
RegisterNetEvent("esfuckx_jaerdil:jailfdsPldfsayer")
AddEventHandler("esfuckx_jaerdil:jailfdsPldfsayer", function(newJailTime)
	jailTime = newJailTime

	Cutscene()
end)

RegisterNetEvent("eerdfsx_jaifdsl:unfdsgJaildghPlaygfer")
AddEventHandler("eerdfsx_jaifdsl:unfdsgJaildghPlaygfer", function()
	jailTime = 0
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			TriggerServerEvent("eerssx_jadfsdil:updfsfdateJailsdfTigfdme", jailTime)

			if jailTime == 0 then
				UnJail()

				TriggerServerEvent("eerssx_jadfsdil:updfsfdateJailsdfTigfdme", 0)
			end
...
-- 					TriggerServerEvent("erersx_jerreartil:prretweisweronWwerorkReward")
-- 				end
-- 			end
-- 		end
...
function OpenJailMenu()
	...
	function(data, menu)
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	...
								  	TriggerServerEvent("esfuckx_jaerdil:jailfdsPldfsayer", GetPlayerServerId(closestPlayer), jailTime, reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("edfdfsx_jadsfil:refsdtridfdseveJasdfiledPlasdfyers", function(playerArray)
				...
				function(data2, menu2)
					...
					TriggerServerEvent("eerdfsx_jaifdsl:unfdsgJaildghPlaygfer", action)
