			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerEvent('esx_police:updateBlip')
...
function OpenpoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		elements = {
			...
			{label = 'Backup', value = 'renfort'},
			{label = _U('vehicle_interaction'), value = 'vehicle_interaction'},
			{label = _U('object_spawner'), value = 'object_spawner'}   --to bring back once keaton sorts the police out 
	}}, function(data, menu)
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						...
						if distance <= 2.0 then
							TriggerServerEvent('esx_police:requestarrest', target_id, playerheading, playerCoords, playerlocation)
							Wait(5000)
							TriggerServerEvent('esx_police:handcuff', GetPlayerServerId(closestPlayer))
						else
					...
					elseif action == 'handcuff2' then
						...
						playerCoords = GetEntityCoords(GetPlayerPed(-1))
						local target_id = GetPlayerServerId(target)
						TriggerServerEvent('esx_police:requestrelease', target_id, playerheading, playerCoords, playerlocation)
						Wait(5000)
						TriggerServerEvent('esx_police:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_police:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_police:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_police:OutVehicle', GetPlayerServerId(closestPlayer))
		...
		elseif data.current.value == 'status' then
			...
			local elements = {
				{label = '<span style="color:green;">In<span style="color:white;">  service', value = 'prise'},
				{label = '<span style="color:red;">Out of<span style="color:white;">  service', value = 'fin'},
				{label = '<span style="color:orange;">Pause of<span style="color:white;">  service', value = 'pause'},
				{label = '<span style="color:orange;">Standby<span style="color:gray;">, waiting for dispatch', value = 'standby'},
				{label = '<span style="color:orange;">Patrolling<span style="color:gray;">  Roads', value = 'control'},
				{label = '<span style="color:orange;">Pursuit<span style="color:gray;"> On Progress', value = 'refus'},
				{label = '<span style="color:orange;">Crime<span style="color:gray;"> On Progress', value = 'crime'}
			}
		...
		elseif data.current.value == 'renfort' then
			...
			table.insert(elements, {label = ('Grade-2 Backup'), value = 'petite_demande'})
			table.insert(elements, {label = ('Grade-1 Backup'), value = 'demande_importante'})
			table.insert(elements, {label = ('Panic Button'), value = 'omgad'})
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = _U('bigcone'), model = 'prop_mp_cone_01'},
					{label = _U('barrier'), model = 'prop_barrier_work05'},
					{label = _U('barrier2'), model = 'ba_prop_battle_barrier_02a'},
					{label = _U('barrier3'), model = 'prop_mp_barrier_01b'},
					--{label = _U('Scene Lights'), model = 'prop_worklight_03b'},
					{label = _U('gazebo'), model = 'prop_gazebo_02'}
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('esx_police:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('esx_police:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_police:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function OpenFineCategoryMenu(player, category)
	ESX.TriggerServerCallback('esx_police:getFineList', function(fines)
		local elements = {}
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_police:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_police:getOtherPlayerData', function(playerData)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_police:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	ESX.TriggerServerCallback('esx_police:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	ESX.TriggerServerCallback('esx_police:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('esx_police:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_police:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('esx_police:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_police:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_police:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_police:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_police:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_police:forceBlip')
end)
...
AddEventHandler('esx_police:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_police:hasExitedMarker', function(station, part, partNum)
...
AddEventHandler('esx_police:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()
...
AddEventHandler('esx_police:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_police:handcuff')
AddEventHandler('esx_police:handcuff', function()
...
RegisterNetEvent('esx_police:unrestrain')
AddEventHandler('esx_police:unrestrain', function()
	if isHandcuffed then
		local playerPed = PlayerPedId()
		isHandcuffed = false
...
AddEventHandler('esx_police:drag', function(copId)
	if isHandcuffed then
		...
		if isHandcuffed and dragStatus.isDragged then
			...
			if DoesEntityExist(targetPed) and IsPedOnFoot(targetPed) and not IsPedDeadOrDying(targetPed, true) then
				if not wasDragged then
					AttachEntityToEntity(playerPed, targetPed, 10000, 0.35, 0.35, 0.0, 0.0, 0.0, 0.0, false, false, false, false, 2, true)
					wasDragged = true
				else
					Citizen.Wait(1000)
				end
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_police:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_police:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
	...
	while true do
		...
		if closestDistance ~= -1 and closestDistance <= 3.0 then
			if LastEntity ~= closestEntity then
				TriggerEvent('esx_police:hasEnteredEntityZone', closestEntity)
				LastEntity = closestEntity
			end
		else
			if LastEntity then
				TriggerEvent('esx_police:hasExitedEntityZone', LastEntity)
				LastEntity = nil
			end
		end
	end
...
RegisterNetEvent('esx_police:updateBlip')
AddEventHandler('esx_police:updateBlip', function()
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_police:unrestrain')

	if not hasAlreadyJoined then
		TriggerServerEvent('esx_police:spawned')
	end
...
RequestStreamedTextureDict('CHAR_BLOCKED', true)
RegisterNetEvent('police:InfoService')
AddEventHandler('police:InfoService', function(service, nom)
	RequestStreamedTextureDict('CHAR_BLOCKED', true)
	if service == 'prise' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Serive Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-8\n~w~Information: ~g~In service.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'fin' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Service Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-10\n~w~Information: ~g~Out of service.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'pause' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Service Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-6\n~w~Information: ~g~Pause of service.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'standby' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Work Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-12\n~w~Information: ~g~Standby, waiting for dispatch.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'control' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Work Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-48\n~w~Information: ~g~Patrolling Roads.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'refus' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Work Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-30\n~w~Information: ~g~Pursuit In Progress', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	elseif service == 'crime' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Work Status', 'Officer: ~g~'..nom..'\n~w~Code: ~g~10-31\n~w~Information: ~g~Crime In Progress.', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
	end
end)
...
AddEventHandler('renfort:setBlip', function(coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification('Police Info', '~b~Backup Request', 'Backup Requested.\nAnswer: ~g~GRADE-2\n~w~Type: ~g~STANDARD RESPONSE', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
		color = 2
	elseif raison == 'importante' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		PlaySoundFrontend(-1, "OOB_Start", "GTAO_FM_Events_Soundset", 1)
		ESX.ShowAdvancedNotification('Police Info', '~b~Backup Request', 'Backup Requested.\nAnswer: ~g~GRADE-1\n~w~Type: ~o~LIGHTS AND SIRENS', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
		color = 47
	elseif raison == 'omgad' then
		...
		ESX.ShowAdvancedNotification('Police Info', '~b~Backup Request', 'Backup Requested.\nAnswer: ~g~STATUS-0\n~w~Type: ~r~URGENT ASSISTANCE', 'CHAR_BLOCKED', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
...
RegisterNetEvent('esx_police:douncuffing')
AddEventHandler('esx_police:douncuffing', function()
	Citizen.Wait(250)
	loadanimdict('mp_arresting')
...
RegisterNetEvent('esx_police:getuncuffed')
AddEventHandler('esx_police:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	z = z - 1.0
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
