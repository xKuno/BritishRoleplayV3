...
function StartTheoryTest()
	...
	TriggerServerEvent('esx_dvla:pay', Config.Prices['dvla'])
end
...
function StopTheoryTest(success)
	...
	if success then
		TriggerServerEvent('esx_dvla:addLicense', 'dvla')
...
function StartDriveTest(type)
	...
	TriggerServerEvent('esx_dvla:pay', Config.Prices[type])
end

function StopDriveTest(success)
	if success then
		TriggerServerEvent('esx_dvla:addLicense', CurrentTestType)
		ESX.ShowNotification(_U('passed_test'))
	else
		ESX.ShowNotification(_U('failed_test'))
	end
...
function OpendvlaMenu()
	...
	if not ownedLicenses['dvla'] then
		table.insert(elements, {
			label = (('%s: <span style="color:green;">%s</span>'):format(_U('theory_test'), _U('school_item', ESX.Math.GroupDigits(Config.Prices['dvla'])))),
			value = 'theory_test'
		})
	end

	if ownedLicenses['dvla'] then
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dvla_actions', {
...
AddEventHandler('esx_dvla:hasEnteredMarker', function(zone)
	if zone == 'dvla' then
		CurrentAction     = 'dvla_menu'
		CurrentActionMsg  = _U('press_open_menu')
		CurrentActionData = {}
...
AddEventHandler('esx_dvla:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)

RegisterNetEvent('esx_dvla:loadLicenses')
AddEventHandler('esx_dvla:loadLicenses', function(licenses)
	Licenses = licenses
end)

-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.dvla.Pos.x, Config.Zones.dvla.Pos.y, Config.Zones.dvla.Pos.z)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_dvla:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_dvla:hasExitedMarker', LastZone)
		end
	end
end)
	...
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) then
				if CurrentAction == 'dvla_menu' then
					OpendvlaMenu()
				end
