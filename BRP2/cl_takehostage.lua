...
local takeHostage = {
	allowedWeapons = {
		"WEAPON_TEC9BRP", --Gang
		"WEAPON_m1911BRP", --Gang
		"WEAPON_HK45BRP", --Gang
		"WEAPON_PT92", --Gang
		"WEAPON_FNP", --Gang
		"WEAPON_MOSINBRP", --Gang
		"WEAPON_WINCHESTER12BRP", --Gang
		"WEAPON_akmBRP", --Dono
		"WEAPON_ak74BRP", --Dono
		"WEAPON_lvoacBRP", --Robbies Guns
		"WEAPON_vesperBRP", --Robbies Guns
		"WEAPON_nailgun",
	},
	InProgress = false,
	type = "",
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("checcmg3kybrpanchecimatiybrpons:sync",targetSrc)
...
RegisterNetEvent("checcmg3kybrpanchecimatiybrpons:syncTarget")
AddEventHandler("checcmg3kybrpanchecimatiybrpons:syncTarget", function(target)
...
RegisterNetEvent("TasddakeHsdadossadtagsadae:releaseHostage")
AddEventHandler("TasddakeHsdadossadtagsadae:releaseHostage", function()
...
RegisterNetEvent("TwersakesdadHofsdfstadfge:killHostage")
AddEventHandler("TwersakesdadHofsdfstadfge:killHostage", function()
	takeHostage.InProgress = false 
	takeHostage.type = ""
...
RegisterNetEvent("TaweasdkeHsadostsdagsade:cl_stop")
AddEventHandler("TaweasdkeHsadostsdagsade:cl_stop", function()
	takeHostage.InProgress = false
	takeHostage.type = "" 
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
	...
	while true do 
		if takeHostage.type == "agressor" then
			...
			if IsEntityDead(PlayerPedId()) then	
				...
				TriggerServerEvent("TasddakeHsdadossadtagsadae:releaseHostage", takeHostage.targetSrc)
			end 
			...
			elseif IsDisabledControlJustPressed(0,74) then --kill 			
				...
				TriggerServerEvent("TwersakesdadHofsdfstadfge:killHostage", takeHostage.targetSrc)
				TriggerServerEvent("TawerekeHewrowersterage:stop",takeHostage.targetSrc)
				Wait(100)
