...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s]^4 was prioritized and placed %d/%d in queue", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] ^4was placed %d/%d in queue", tmp.name, ids[1], _pos, queueCount))
    end
...
function Queue:AddToConnecting(ids, ignorePos, autoRemove, done)
    local function remove()
        ...
        Queue:DebugPrint("^4Player could not be added to the connecting list")
    end
    ...
    if type(id) == "table" then
        for _id, power in pairs(id) do
            if _id and type(_id) == "string" and power and type(power) == "number" then
                ...
                Queue:DebugPrint("^4Error adding a priority id, invalid data passed")
                return false
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if not ids then
        ...
        CancelEvent()
        Queue:DebugPrint("^4Dropped " .. name .. ", ^4couldn't retrieve any of their id's")
        return
    end
    ...
    Queue:CanJoin(src, function(reason)
        ...
        if reason then
            -- prevent joining
            allow = false
            done(reason and tostring(reason) or "^4You were blocked from joining")
            Queue:RemoveFromQueue(ids)
            Queue:RemoveFromConnecting(ids)
            Queue:DebugPrint(string_format("%s[%s] ^4was blocked from joining; Reason: %s", name, ids[1], reason))
            CancelEvent()
            return
        end
    ...
    if Queue:IsInQueue(ids) then
        ...
        Queue:DebugPrint(string_format("%s[%s] ^4has rejoined queue after cancelling", name, ids[1]))
    else
        Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    if Queue:NotFull(true) and _Queue.JoinDelay <= GetGameTimer() then
        ...
        Queue:DebugPrint(name .. "[" .. ids[1] .. "] ^4is loading into the server")
    ...
    while true do
        ...
        if not data or not data.deferrals or not data.source or not pos then
            remove("^4[Queue] Removed from queue, queue data invalid :(")
            Queue:DebugPrint(tostring(name .. "[" .. ids[1] .. "] ^4was removed from the queue because they had invalid data"))
            return
        end
        ...
        if data.timeout >= Config.QueueTimeOut and os_time() - connectTime > 5 then
            remove("^4[Queue] Removed due to timeout")
            Queue:DebugPrint(name .. "[" .. ids[1] .. "] ^4was removed from the queue because they timed out")
            return
        end
    ...
    while true do
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and not endPoint) or data.timeout >= Config.ConnectTimeOut) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                remove(data)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] ^4was removed from the connecting queue because they timed out")
            else
                i = i + 1
            end
        end
...
if Config.DisableHardCap then
    Queue:DebugPrint("^4[connectqueue] Disabling hardcap ^4")
...
commands.addq = function()
    Queue:DebugPrint("^4ADDED DEBUG QUEUE")
...
commands.removeq = function(args)
    ...
    Queue:DebugPrint("^4REMOVED " .. tostring(name) .. " ^4FROM THE QUEUE")
end
...
commands.addc = function()
    ...
    Queue:DebugPrint("^4ADDED DEBUG CONNECTING QUEUE")
end
...
commands.removec = function(args)
    ...
    Queue:DebugPrint("^4REMOVED " .. tostring(name) .. " ^4FROM THE CONNECTING LIST")
end

commands.printc = function()
    Queue:DebugPrint("^4CURRENT CONNECTING LIST")
...
commands.printp = function()
    Queue:DebugPrint("^4CURRENT PRIORITY LIST")
...
commands.printcount = function()
    Queue:DebugPrint("^4Player Count: " .. Queue:GetPlayerCount())
end

commands.printtp = function()
    Queue:DebugPrint("^4CURRENT TEMP PRIORITY LIST")
...
commands.removetp = function(args)
    ...
    Queue:DebugPrint("^4REMOVED " .. args[1] .. " ^4FROM THE TEMP PRIORITY LIST")
end
...
commands.setpos = function(args)
    ...
    Queue:DebugPrint("SET " .. data.name .. "'^4s QUEUE POSITION TO: " .. args[2])
end
